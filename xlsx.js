...
function fmt_is_date(fmt) {
	...
	while(i < fmt.length) {
		switch((c = fmt.charAt(i))) {
			...
			case 'A': case 'a': case '訝�':
				if(fmt.substr(i, 3).toUpperCase() === "A/P") return true;
				if(fmt.substr(i, 5).toUpperCase() === "AM/PM") return true;
				if(fmt.substr(i, 5).toUpperCase() === "訝듿뜄/訝뗥뜄") return true;
				++i; break;
			case '[':
				o = c;
	...
	while(i < fmt.length) {
		switch((c = fmt.charAt(i))) {
			...
			case 'A': case 'a': case '訝�':
				...
				else if(fmt.substr(i,5).toUpperCase() === "訝듿뜄/訝뗥뜄") { if(dt!=null) q.v = dt.H >= 12 ? "訝뗥뜄" : "訝듿뜄"; q.t = 'T'; i+=5; hr='h'; }
				else { q.t = "t"; ++i; }
			...
			default:
				if(",$-+/():!^&'~{}<>=�촡cfijklopqrtuvwxzP".indexOf(c) === -1) throw new Error('unrecognized character ' + c + ' in ' + fmt);
...
function load_props_pairs(HP, TOP, props, opts) {
	...
	if(parts.length > 0) for(var i = 0; i !== v.length; i += 2) {
		...
		switch(v[i].v) {
			...
			case "������瘻 鬧����":
			case "Arbeitsbl채tter":
			case "횉al캇힊ma Sayfalar캇":
			case "Feuilles de calcul":
			case "Fogli di lavoro":
			case "Folhas de c찼lculo":
			case "Planilhas":
			case "Regneark":
			case "Hojas de c찼lculo":
			case "Werkbladen":
...
var SYLK = (function() {
	...
	var sylk_escapes = ({
		AA:'�', BA:'횁', CA:'횂', DA:195, HA:'횆', JA:197,
		AE:'횊', BE:'횋', CE:'횎',         HE:'횏',
		AI:'횑', BI:'횒', CI:'횓',         HI:'횕',
		AO:'횘', BO:'횙', CO:'횚', DO:213, HO:'횜',
		AU:'횢', BU:'횣', CU:'횤',         HU:'횥',
		Aa:'횪', Ba:'찼', Ca:'창', Da:227, Ha:'채', Ja:229,
		Ae:'챔', Be:'챕', Ce:'챗',         He:'챘',
		Ai:'챙', Bi:'챠', Ci:'챤',         Hi:'챦',
		Ao:'챵', Bo:'처', Co:'척', Do:245, Ho:'철',
		Au:'첫', Bu:'첬', Cu:'청',         Hu:'체',
		KC:'횉', Kc:'챌', q:'챈',  z:'흹',  a:'횈',  j:'흸',
...
function write_theme(Themes, opts) {
	...
	o[o.length] =     '<a:font script="Jpan" typeface="竊�설 竊겹궡�룔긿��"/>';
	o[o.length] =     '<a:font script="Hang" typeface="留묒� 怨좊뵓"/>';
	o[o.length] =     '<a:font script="Hans" typeface="若뗤퐪"/>';
	o[o.length] =     '<a:font script="Hant" typeface="�곁눗�롩쳱"/>';
...
var parse_content_xml = (function() {
	...
	return function pcx(d, _opts) {
		...
		while((Rn = xlmlregex.exec(str))) switch((Rn[3]=Rn[3].replace(/_.*$/,""))) {
			...
			case 'table': case '藥δ퐳烏�': // 9.1.2 <table:table>
				if(Rn[1]==='/') {
					...
					sheetag.name = sheetag['�띸㎞'] || sheetag.name;
			...
			case 'table-row-group': // 9.1.9 <table:table-row-group>
				...
				break;
			case 'table-row': case '烏�': // 9.1.3 <table:table-row>
				if(Rn[1] === '/') { R+=rowpeat; rowpeat = 1; break; }
				rowtag = parsexmltag(Rn[0], false);
				if(rowtag['烏뚦뤇']) R = rowtag['烏뚦뤇'] - 1; else if(R == -1) R = 0;
			...
			case 'table-cell': case '�경뜮':
				if(Rn[0].charAt(Rn[0].length-2) === '/') {
					...
					if((ctag['�경뜮映삣엹'] || ctag['value-type']) == "string") {
					...
					ctag = parsexmltag(Rn[0], false);
					comments = []; comment = ({});
					q = ({t:ctag['�경뜮映삣엹'] || ctag['value-type'], v:null});
					if(opts.cellFormula) {
					...
					switch(q.t) {
						...
						case 'number': q.t = 'n'; q.v = parseFloat(ctag['�경뜮�겼��']); break;
						default:
			...
			case 'document-content': case '�드춴烏ⓩ졏�뉑。': // 3.1.3.2 <office:document-content>
			case 'spreadsheet': case '訝삡퐪': // 3.7 <office:spreadsheet>
			...
			/* ignore state */
			case 'meta': case '�껅빊��': // TODO: <office:meta> <uof:�껅빊��> FODS/UOF
			...
			case 'body': case '�드춴烏ⓩ졏': break; // 3.3 16.9.6 19.726.3
			...
			case 'title': case '�뉔쥦': break; // <*:title> OR <uof:�뉔쥦>
